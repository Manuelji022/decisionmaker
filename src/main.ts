...
function applyTranslations() {
  ...
  const addOptionEl = document.getElementById(
    "add-option"
  ...
  if (addOptionEl) {
    addOptionEl.textContent =
      optionCount > 2 ? i18n.t("removeOption") : i18n.t("addOption");
  }
...
function updateAddOptionButton() {
  ...
  if (addOptionEl) {
    ...
    addOptionEl.textContent =
      optionCount >= maxOptions ? i18n.t("removeOption") : i18n.t("addOption");
  }
}
...
function addOption() {
  ...
  const additionalOptionsContainer = document.querySelector(
...
function removeOption() {
  ...
  const currentOptionEl = document.getElementById(
    `option${optionCount}`
  ) as HTMLInputElement;
  if (currentOptionEl) {
